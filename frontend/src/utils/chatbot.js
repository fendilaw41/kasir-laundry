import { db } from '../db';
import { withNewSyncMeta, withUpdatedSyncMeta } from '../../src/db/syncMeta.js';
...
const generateInvoiceId = async () => {
  ...
  return `${deviceId}-${nextNumber.toString().padStart(5, '0')}`;
};
...
export async function parseChatCommand(message, user) {
  ...
  try {
        ...
        if (notFound) {
          ...
          if (pelangganNama !== 'Via Chat') {
            ...
            if (existingPelanggan) {
              ...
              pelangganId = await db.pelanggan.add(withNewSyncMeta({ nama: pelangganNama, hp: hp, alamat: '-' }));
            }
          }
          ...
          await db.orders.add(withNewSyncMeta(orderData));
        ...
        if (itemMatch) {
          ...
          if (qty > 0) {
            ...
            if (matchedInv) {
              ...
              await db.inventory.update(matchedInv.id, withUpdatedSyncMeta({
              ...
              await db.inventory.add(withNewSyncMeta({
                nama: finalName,
                qty: qty,
                unit: unit,
                status: 'pending',
            ...
            for (const item of pendingItems) {
              ...
              if (item.qty > 0) {
                await db.inventoryEvents.add(withNewSyncMeta({
                  ...
                  type: 'ADD',
                  qty: item.qty,
                  note: 'Approved inventory request via Chatbot',
                  createdBy: user.fullname || user.username
                }));
              }
            }
    ...
    // 4. Cek intent: Partial Approve Inventory -> "acc <barang>"
      ...
      if (user.role === 'owner') {
        ...
        if (pendingItems.length === 0) {
          ...
          if (matchedItem) {
            await db.transaction('rw', [db.inventory, db.inventoryEvents], async () => {
              await db.inventory.update(matchedItem.id, withUpdatedSyncMeta({
                status: 'approved',
                qty: 0
              }));
    ...
    // 6. Cek intent: Undo Transaksi Terakhir -> "undo trx terakhir"
      ...
      if (lastOrder) {
        if (lastOrder.status === 'Proses') {
          await db.orders.update(lastOrder.id, withUpdatedSyncMeta({ status: 'Dibatalkan' }));
      ...
      if (newBerat <= 0) {
        ...
        if (lastOrder && lastOrder.items && lastOrder.items.length > 0) {
          ...
          if (matchedProduct) {
            ...
            await db.orders.update(lastOrder.id, withUpdatedSyncMeta({
              items: updatedItems,
              subtotal: newTotal,
              total: newTotal
            }));
    ...
    // 8. Cek intent: Tambah Pelanggan -> "Nama-NomorHP"
      ...
      if (existing) {
        ...
        await db.pelanggan.add(withNewSyncMeta({ nama, hp, alamat: '-' }));
  ...
  await db.chatLog.add(withNewSyncMeta({
    pesan: message,
    intent,
    payload,
    status,
