...
const OrderDetail = () => {
  ...
  const toggleLunas = async () => {
    if (order.statusBayar === 'Lunas') {
      await updateOrder(order.id, { statusBayar: 'Belum Lunas', bayar: 0, kembalian: 0 });
    } else {
      await updateOrder(order.id, { statusBayar: 'Lunas', metodeBayar: 'Tunai', bayar: order.total, kembalian: 0 });
    }
  };
  ...
  const sendWhatsApp = () => {
    ...
    const waData = formatWhatsAppMessage(order, pelanggan, settings);
    if (!waData) {
  ...
  const saveHpAndSend = async () => {
    ...
    setTimeout(() => {
      const waData = formatWhatsAppMessage(order, updatedPelanggan, settings);
      ...
      <div className="order-detail-wrapper pb-5">
        ...
        <div className="card shadow-sm border-0 mb-3 overflow-hidden">
          ...
          <div className={`p-4 text-center text-white position-relative ${order.isPriority ? 'bg-danger' : 'bg-primary'}`}>
            ...
            <h5 className="text-white fw-bold mb-0">{settings?.namaLaundry || 'Kasir Laundry'}</h5>
            <small className="opacity-75">{settings?.motto || 'Solusi Laundry Bersih & Cepat'}</small>
          </div>
            ...
            <div className="dropdown">
              ...
              <ul className="dropdown-menu dropdown-menu-end shadow border-0 w-100">
                {order.statusBayar !== 'Lunas' && (
                  <li><button className="dropdown-item py-2 small" onClick={() => navigate('/pembayaran', { state: { orderData: order, isPaymentOnly: true } })}><i className="bi bi-wallet2 me-2 text-primary"></i> Bayar</button></li>
                )}
                {order.status !== 'Ambil' && (
                  <li>
                    <button className="dropdown-item py-2 small" onClick={toggleLunas}>
                      {order.statusBayar === 'Lunas' ? (
                        <><i className="bi bi-x-circle me-2 text-danger"></i> Jadikan Tidak Lunas</>
                      ) : (
                        <><i className="bi bi-check-circle me-2 text-success"></i> Jadikan Lunas</>
                      )}
                    </button>
                  </li>
                )}
